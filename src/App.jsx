function App() {
  ...
  const skills = ['React', 'Tailwind CSS', 'JavaScript', 'HTML', 'CSS', 'C']
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="mx-auto max-w-6xl px-6 py-6 sm:px-8">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xl font-semibold tracking-wide text-emerald-400">Tahamid Hassan Portfolio</p>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-6 pb-16 sm:px-8">
        ...
        <section id="projects" className="mt-20">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-emerald-400">Projects</p>
              <h2 className="mt-3 text-3xl font-semibold text-white">Selected work</h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-slate-400">
              A few highlights from recent work, demonstrating clean design, responsive structure, and polished product presentation.
            </p>
          </div>

          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {projects.map((project) => (
              <article key={project.title} className="group rounded-[1.75rem] border border-white/10 bg-slate-900/80 p-6 transition hover:-translate-y-1 hover:border-emerald-500/30 hover:bg-slate-900/95">
                <div className="flex items-center justify-between gap-4">
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="rounded-full border border-slate-700/80 px-3 py-1 text-xs text-slate-300">{tag}</span>
                  ))}
                </div>
        ...
        <section id="skills" className="mt-20 rounded-4xl border border-white/10 bg-slate-900/80 p-8 shadow-2xl shadow-black/20 backdrop-blur-xl">
          <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-emerald-400">Skills</p>
              <h2 className="mt-4 text-3xl font-semibold text-white">What I use every day.</h2>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {skills.map((skill) => (
                <div key={skill} className="rounded-3xl border border-white/10 bg-slate-950/80 px-5 py-4 text-sm text-slate-200 shadow-sm shadow-black/20">
                  {skill}
                </div>
              ))}
            </div>
